...
![Demo](https://i.imgur.com/dCVTBwz.gif)
*Widget themes can be Changed, dragged and made vertical or horizontal*
![image](https://i.imgur.com/nodjpJE.png)
*You can paste your custom folder path to open your Music*
![image](https://i.imgur.com/Eyx3ht4.png)
*You can apply a your custom color code here, like #0BA6DA*
*/
// ==/WindhawkModReadme==
...
- personalizationWidgetOrientation: horizontal
  ...
  - vertical: Vertical
- materialCustomColor: ""
  $name: Liquid Glass / Acrylic custom color
  $description: Optional custom color for Liquid Glass and Acrylic accents. Use #RRGGBB, RRGGBB, 0xRRGGBB, or #AARRGGBB. Leave empty to use the built-in white.
- musicFolderPath: ""
...
AppearanceTheme g_appearanceTheme = AppearanceTheme::LiquidGlass;
...
bool IsAcrylicTheme(const ThemePalette* theme) {
    return theme && theme->theme == AppearanceTheme::AcrylicTranslucent;
}
...
Color MaterialThemeHeaderColor(const ThemePalette* theme) {
    if (IsAcrylicTheme(theme)) {
        return MaterialAccentColor(theme->header.GetA());
    }
...
Color MaterialIconColor(const ThemePalette* theme, bool liquidGlass, Color fallback) {
    if (IsAcrylicTheme(theme) || liquidGlass) {
...
void ApplyOrientationResizeIfNeeded(bool oldVolumeVertical,
    ...
    if (oldSearchVertical != g_searchWidgetVertical) {
        g_widgets[WidgetSearch].rc = RectForOrientation(
            g_widgets[WidgetSearch].rc, g_searchWidgetVertical, 165.0f, 80.0f);
    ...
    if (oldPersonalizationVertical != g_personalizationWidgetVertical) {
        g_widgets[WidgetPersonalization].rc = RectForOrientation(
            g_widgets[WidgetPersonalization].rc, g_personalizationWidgetVertical, 165.0f, 80.0f);
...
void DrawCalendar(Graphics& g, const Widget& widget, FontFamily& family) {
    ...
    SolidBrush headerText(theme ? theme->textOnHeader : liquidGlass ? LiquidGlassTextColor()     : Color(245, 255, 255, 255));
    SolidBrush text      (theme ? theme->textOnBody   : liquidGlass ? LiquidGlassTextColor(245)  : Color(245, 255, 255, 255));
    SolidBrush muted     (theme ? theme->textOnBody   : liquidGlass ? LiquidGlassTextColor(205)  : Color(205, 255, 255, 255));
...
void DrawClock(Graphics& g, const Widget& widget, FontFamily& family) {
    ...
    Color dialColor     = theme ? MaterialThemeHeaderColor(theme) : liquidGlass ? LiquidGlassTextColor(150) : Color(230, 255, 255, 255);
    Color dialTextColor = theme ? theme->textOnHeader             : liquidGlass ? LiquidGlassTextColor()    : Color(245, 255, 255, 255);
    Pen dialPen(dialColor, 2.0f);
    Pen hourTick  (theme ? dialTextColor : liquidGlass ? LiquidGlassTextColor()    : Color(255, 255, 210, 38), 3.0f);
    Pen minuteTick(theme ? dialTextColor : liquidGlass ? LiquidGlassTextColor(220) : Color(230, 255, 255, 255), 1.2f);
    ...
    Color handColor = theme ? dialTextColor : liquidGlass ? MaterialAccentColor() : Color(255, 35, 125, 255);
...
void DrawMusic(Graphics& g, const Widget& widget) {
    ...
    SolidBrush playGlyph(theme ? theme->textOnHeader : liquidGlass ? MaterialAccentColor() : Color(255, 35, 35, 35));
...
void DrawSlider(Graphics& g, const Widget& widget, const wchar_t* label,
    ...
    SolidBrush text(theme ? theme->textOnAccent : liquidGlass ? LiquidGlassTextColor() : Color(245, 255, 255, 255));
    ...
    SolidBrush track(theme ? theme->body : liquidGlass ? LiquidGlassTextColor(90) : Color(105, 255, 255, 255));
    ...
    SolidBrush fill(theme ? MaterialThemeHeaderColor(theme) : liquidGlass ? MaterialAccentColor() : fillColor);
    DrawRoundedRectangle(g, filled, 7.5f, fill);
    SolidBrush thumb(theme ? theme->textOnAccent : liquidGlass ? LiquidGlassTextColor() : Color(245, 255, 255, 255));
    if (vertical) {
...
void DrawSmallButton(Graphics& g, const Widget& widget, const wchar_t* label,
    ...
    const ThemePalette* theme = CurrentTheme();
    const bool liquidGlass = IsLiquidGlassTheme();
    Color iconColor = MaterialIconColor(theme, liquidGlass, Color(255, 255, 210, 38));
    Font font(&family, 13, FontStyleRegular, UnitPixel);
    SolidBrush text(theme ? theme->textOnAccent : liquidGlass ? LiquidGlassTextColor() : Color(245, 255, 255, 255));
    const bool vertical = IsWidgetVertical(widget.id);
